...
%\DndSetComplexThemeColor[mytheme][mysidebar][myreadaloud][myreadaloudborder][titlered][titlegold][mypagenum]
...
\subsection{More Theme Colors Control}
Use |\DndSetComplexThemeColor| in order to have more precise control over the various theme colors defined within the document class. This command takes seven (7) |<color>| values that map to the following color variables within the theme:

\begin{DndTable}[header=Complex Theme Parameter Map]{cX}
  \textbf{Param \#} & \textbf{Variable Mapping} \\
  1 & \textit{themecolor}, \textit{tablecolor} \\
  2 & \textit{commentcolor}, \textit{sidebarcolor} \\
  3 & \textit{readaloudcolor} (DndReadAloud background) \\
  4 & \textit{readaloudbordercolor} \\
  5 & \textit{titlecolor} \\
  6 & \textit{titlerulecolor} \\
  7 & \textit{pagecolor} (page numbers and footer) \\
\end{DndTable}
...
An example of these settings for a theme similar to the \textit{Ghosts of Saltmarsh} sourcebook:
